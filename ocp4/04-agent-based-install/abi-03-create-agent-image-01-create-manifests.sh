...
if [[ -f "$MIRROR_REGISTRY_CRT_FILE" ]]; then
    ...
    for var in "MIRROR_REGISTRY" "LOCAL_REPOSITORY_NAME" "PULL_SECRET"; do
        if [[ -z "${!var}" ]]; then
            echo "[ERROR] Required variable '$var' is empty when MIRROR_REGISTRY_CRT_FILE is set. Exiting..."
            exit 1
        fi
    done
    ### Validate certificate file
    if [[ ! -s "$MIRROR_REGISTRY_CRT_FILE" ]]; then
        echo "[ERROR] MIRROR_REGISTRY_CRT_FILE '$MIRROR_REGISTRY_CRT_FILE' is empty. Exiting..."
        exit 1
    fi
    if ! grep -q "^-----BEGIN CERTIFICATE-----" "$MIRROR_REGISTRY_CRT_FILE"; then
        echo "[ERROR] MIRROR_REGISTRY_CRT_FILE '$MIRROR_REGISTRY_CRT_FILE' does not contain a valid certificate. Exiting..."
        exit 1
    fi
