...
if [[ -f "$MIRROR_REGISTRY_CRT_FILE" ]]; then
    ...
    printf "%-8s%-80s\n" "[INFO]" "--- Validating Disconnected Installation Prerequisites ..."
    printf "%-8s%-80s\n" "[INFO]" "    - Using release image override: $OPENSHIFT_INSTALL_RELEASE_IMAGE_OVERRIDE"
    ...
    ### Validate the mirror registry certificate file.
    if [[ ! -s "$MIRROR_REGISTRY_CRT_FILE" ]]; then
        printf "%-8s%-80s\n" "[ERROR]" "    The mirror registry certificate file '$MIRROR_REGISTRY_CRT_FILE' is empty or not found. Exiting..."
        exit 1
    fi
    if ! grep -q "^-----BEGIN CERTIFICATE-----" "$MIRROR_REGISTRY_CRT_FILE"; then
        printf "%-8s%-80s\n" "[ERROR]" "    The certificate file '$MIRROR_REGISTRY_CRT_FILE' does not appear to be a valid PEM certificate. Exiting..."
        exit 1
    fi

    ### Validate the pull secret file and its JSON format.
    if command -v jq --version >/dev/null 2>&1; then
        if [[ ! -f "$PULL_SECRET" ]] || ! jq . "$PULL_SECRET" >/dev/null 2>&1; then
            printf "%-8s%-80s\n" "[ERROR]" "    The pull secret file '$PULL_SECRET' is missing or is not valid JSON. Exiting..."
            exit 1
        fi
    fi
