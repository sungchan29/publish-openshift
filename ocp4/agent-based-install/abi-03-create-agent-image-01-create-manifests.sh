...
if [[ -n "${MIRROR_REGISTRY_TRUST_FILE:-}" && -f "$MIRROR_REGISTRY_TRUST_FILE" ]]; then
    ...
    if [[ -f "$MIRROR_REGISTRY_TRUST_FILE" ]]; then
        ...
        if command -v podman >/dev/null 2>&1; then
            if ! podman pull "$OPENSHIFT_INSTALL_RELEASE_IMAGE_OVERRIDE" --authfile "$PULL_SECRET" >/dev/null 2>&1; then
            ...
            if ! podman inspect "$OPENSHIFT_INSTALL_RELEASE_IMAGE_OVERRIDE" --authfile "$PULL_SECRET"; then
