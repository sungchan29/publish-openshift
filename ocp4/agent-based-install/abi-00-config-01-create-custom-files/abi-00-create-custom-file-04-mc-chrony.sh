...
if [[ -n "$NTP_SERVER_01" ]]; then
    ...
    for role in "master" "worker"; do
        cat << EOF > $BUTANE_BU_DIR/99-${role}-custom-chrony.bu
variant: openshift
...
EOF
        ./butane $BUTANE_BU_DIR/99-${role}-custom-chrony.bu -o $ADDITIONAL_MANIFEST/99-${role}-custom-chrony.yaml
    done
    if [[ $? -eq 0 ]]; then
        echo "[INFO] Successfully executed : $(dirname "$(realpath "$0")")/$(basename "$0")"
    else
        echo "[ERROR] Failed to patch MachineConfig(Chrony)."
    fi
else
