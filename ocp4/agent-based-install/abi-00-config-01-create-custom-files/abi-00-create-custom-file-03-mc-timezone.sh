...
cat << EOF > $BUTANE_BU_DIR/99-${role}-custom-timezone-configuration.bu
variant: openshift
...
EOF
    ./butane $BUTANE_BU_DIR/99-${role}-custom-timezone-configuration.bu -o $ADDITIONAL_MANIFEST/99-${role}-custom-timezone-configuration.yaml
done
...
if [[ $? -eq 0 ]]; then
    ...
    echo "[ERROR] Failed to patch MachineConfig(TimeZone)."
fi