...
options {
        ...
        listen-on port 53 { 172.16.120.28; };
        ...
        allow-query     { localhost; 10.0.0.0/24; 172.16.120.0/24; };
...
\$TTL 86400
@               IN      SOA     dns.cloudpang.lan. hostmaster.cloudpang.lan. (
                                2025103001 ; Serial
                                21600      ; Refresh
                                3600       ; Retry
                                604800     ; Expire
                                86400 )    ; Minimum TTL
;
                IN      NS      dns.cloudpang.lan.

dns             IN      A       172.16.120.29
registry        IN      A       172.16.120.28
gitlab          IN      A       172.16.120.28
EOF
```
...
api             IN      A       172.16.120.100
api-int         IN      A       172.16.120.100
*.apps          IN      A       172.16.120.100

sno             IN      A       172.16.120.100
EOF
```
...
api             IN      A       172.16.120.28
api-int         IN      A       172.16.120.28
*.apps          IN      A       172.16.120.28

mst01           IN      A       172.16.120.111
mst02           IN      A       172.16.120.112
mst03           IN      A       172.16.120.113
ifr01           IN      A       172.16.120.121
ifr02           IN      A       172.16.120.122
wkr01           IN      A       172.16.120.131
wkr02           IN      A       172.16.120.132
EOF
```

### 역방향 존 파일 (IP → 도메인)

`120.16.172-in-addr.zone` 파일 생성:

```bash
cat <<EOF > /var/named/120.16.172-in-addr.zone
\$ORIGIN 120.16.172.in-addr.arpa.
\$TTL 86400
@               IN      SOA     dns.cloudpang.lan. hostmaster.cloudpang.lan. (
                                2025103001 ; Serial
                                21600      ; Refresh
...
131             IN      PTR       wkr01.ocp4-mgc01.cloudpang.lan.
132             IN      PTR       wkr02.ocp4-mgc01.cloudpang.lan.
EOF
```
...
chown root:named /var/named/120.16.172-in-addr.zone
...
chcon --reference=/var/named/named.empty /var/named/120.16.172-in-addr.zone
```
...
/usr/sbin/named-checkzone 120.16.172.in-addr.arpa  /var/named/120.16.172-in-addr.zone
```
