...
# Allow config file to override, otherwise default to the "nodes" iso
...
[[ ! -f "$iso_file" ]] && { printf "%-8s%-80s\n" "[ERROR]" "ISO not found: '$iso_file'. Run node ISO script first." >&2; exit 1; }
...
for vminfo in "${VM_INFO_LIST[@]}"; do
    ...
    if virsh --connect "$qemu_connect" dominfo "$vmname" &>/dev/null; then
        printf "%-8s%-80s\n" "[INFO]" "VM '$vmname' created successfully on '$host'." >&2
    else
        printf "%-8s%-80s\n" "[WARN]" "  VM '$vmname' not found on '$host' (check log: $log_file)" >&2
    fi
    printf "%-8s%-80s\n" "[INFO]" "  Log: $log_file" >&2
