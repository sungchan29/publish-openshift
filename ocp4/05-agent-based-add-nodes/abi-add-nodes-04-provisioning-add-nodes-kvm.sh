...
### ---------------------------------------------------------------------------------
### OpenShift Agent-Based VM Provisioning (Sequential + Dedicated Log Directory)
### - This script is for ADDING NODES
### - br-ocp auto-assigned via subnet matching
### - Uses Pure Bash for subnet calculation (No ipcalc dependency)
### - VMs created sequentially to avoid libvirt pool conflicts
### - Logs and PIDs saved to: ./create-kvm-log/
### - Directory cleaned at start
### ---------------------------------------------------------------------------------
...
### VM Configuration
### ---------------------------------------------------------------------------------
### NOTE: VM_INFO_LIST is loaded from the config file, but can be overridden here if needed.
VM_INFO_LIST=(
     "localhost--wkr01--8--8192--100--0"
)

### Default: /var/lib/libvirt/images
VIRT_DIR=""

### Default: ./${CLUSTER_NAME}/${CLUSTER_NAME}-v${OCP_VERSION}_nodes.x86_64.iso
ISO_FILE=""
...
###   1. iso_src: Full path to the source file (e.g., ./ocp4-cluster-v4.20...iso)
###   2. iso_name: Only the filename (e.g., ocp4-cluster-v4.20...iso) for destination
iso_src="${ISO_FILE:-${CLUSTER_NAME}/${CLUSTER_NAME}-v${OCP_VERSION}_nodes.x86_64.iso}"
...
### ---------------------------------------------------------------------------------
### Process VM Provisioning (Sequential)
### ---------------------------------------------------------------------------------

printf "%-8s%-80s\n" "[INFO]" "=== Starting VM Provisioning (Sequential) ===" >&2
for vminfo in "${VM_INFO_LIST[@]}"; do
    [[ -z "$vminfo" ]] && continue
...
printf "%-8s%-80s\n" "[INFO]" "Next: Check logs and wait for nodes to join cluster." >&2

### Optional Command for CSR Approval
