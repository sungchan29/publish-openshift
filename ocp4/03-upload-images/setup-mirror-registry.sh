...
OC_MIRROR_CACHE_DIR="${OC_MIRROR_CACHE_DIR:-"$HOME/.cache/oc-mirror"}"
...
extract_oc_mirror() {
    local file="$1"
    local tool="$2"
    printf "%-8s%-80s\n" "[INFO]" "--- Extracting '$tool' from '$file'..."
    tar xf "$file" -C ./ "$tool" || {
...
case "$image_type" in
    ...
    "add")
        ...
        find "${OCP_ADD_IMAGES_DIR}" -maxdepth 1 -name "localhost_*.tar" -print0 | while IFS= read -r -d $'\0' tar_file; do
            ### 1. Load images from the tarball and save the loaded image names to a variable.
            ###    Extract just the image names from the 'podman load' output (e.g., "Loaded image: localhost/image:tag").
            printf "%-8s%-80s\n" "[INFO]" "    -- Loading image from '$(basename "$tar_file")'..."
            loaded_images=$(podman load -i "$tar_file" | grep "Loaded image" | awk '{print $3}')

            if [[ -z "$loaded_images" ]]; then
                printf "%-8s%-80s\n" "[WARN]" "      No images were loaded from '$(basename "$tar_file")'. Skipping."
                continue
            fi

            ### 2. Tag and push only the images that were just loaded.
            for image in $loaded_images; do
                ...
                podman tag "$image" "$new_tag" || {
                    printf "%-8s%-80s\n" "[ERROR]" "       Failed to tag image '$image'. Skipping..." >&2
                    continue 2 ### Continue to the next tar file in the outer loop.
                }

                printf "%-8s%-80s\n" "[INFO]" "    -- Pushing image '$new_tag'..."
                podman push "$new_tag" --tls-verify=false || {
                    printf "%-8s%-80s\n" "[ERROR]" "       Failed to push image '$new_tag'. Skipping..." >&2
                    continue 2 ### Continue to the next tar file in the outer loop.
                }
            done
        done
