...
### Defines the target repository name for OpenShift release images and core platform components.
###   Default: "ocp4"
OCP_REPOSITORY=""

### Defines the target repository name for Red Hat provided Operators.
###   Default: "olm-redhat"
OLM_REDHAT_REPOSITORY=""

### Defines the target repository name for Certified Partner Operators.
###   Default: "olm-certified"
OLM_CERTIFIED_REPOSITORY=""

### Defines the target repository name for Community Operators.
###   Default: "olm-community"
...
### Path to the 'oc-mirror' binary.
###   Default: $PWD/oc-mirror
OC_MIRROR_PATH=""
...
OC_MIRROR_PATH="${OC_MIRROR_PATH:-"$PWD/oc-mirror"}"
...
print_usage() {
    ...
    printf "%-8s%-80s\n" "[INFO]" "Usage: $0 <type> (where type is one of: tool, ocp, olm, img)"
    echo ""
    exit 1
}
...
check_oc_mirror() {
    if [[ ! -f "$OC_MIRROR_PATH" ]]; then
        printf "%-8s%-80s\n" "[ERROR]" "    The 'oc-mirror' binary was not found at '$OC_MIRROR_PATH'. Exiting..."
        exit 1
    fi
}
...
case "$image_type" in
    "tool")
        ...
        if [[ -f "$OC_MIRROR_PATH" ]]; then
            printf "%-8s%-80s\n" "[INFO]" "--- Removing existing '$OC_MIRROR_PATH' binary..."
            rm -f oc-mirror
        fi
        ...
        chmod ug+x $OC_MIRROR_PATH

        printf "%-8s%-80s\n" "[INFO]" "    Verification:"
        OC_MIRROR_CMD="$OC_MIRROR_PATH  --v2 version --output=yaml"
        printf "%-8s%-80s\n" "[INFO]" "    --- Executing:"
        printf "%-8s%-80s\n" "[INFO]" "            $OC_MIRROR_CMD"
        $OC_MIRROR_CMD
        OC_MIRROR_CMD="$OC_MIRROR_PATH  --v2 --version"
        printf "%-8s%-80s\n" "[INFO]" "    --- Executing:"
        printf "%-8s%-80s\n" "[INFO]" "            $OC_MIRROR_CMD"
        $OC_MIRROR_CMD
        ;;
    ...
    "img")
        ...
        while IFS= read -r -d $'\0' tar_file; do
            ...
            for image in $loaded_images; do
                ...
                ### Removed 'local' keyword (Global Scope)
