...
### Check for yq (since we might parse catalog.yaml)
...
### Helper functions (Updated to support YAML via yq and strict filtering)
...
get_channels_for_operator() {
    ...
    [[ -s "$f" ]] || { echo ""; return; }
    if [[ "$f" =~ \.ya?ml$ ]]; then
        ### Added strict filtering: non-null name, remove --- lines
...
get_versions_for_channel() {
    ...
    [[ -s "$f" ]] || { echo ""; return; }
    if [[ "$f" =~ \.ya?ml$ ]]; then
        ### Added select(.name) to ensure we don't get empty objects
...
### [IMPROVED] Robust version extraction using Regex
get_extract_version() {
    local input_str="$1"
    if [[ -z "$input_str" ]]; then echo ""; return; fi

    ### Regex to capture semantic version at the end of the string
    ### Matches: .v1.2.3, -1.2.3, .1.2.3, etc.
...
### [FIXED] Initialize associative array to avoid unbound variable errors
...
### [NEW] Helper to sort bundles by extracted version (Numeric Sort)
### Usage: sort_bundles_by_version "${bundle_array[@]}"
sort_bundles_by_version() {
    local bundles=("$@")
    for b in "${bundles[@]}"; do
        local v_extracted
        v_extracted=$(get_extract_version "$b")
        ### Print "version bundle" pairs
        echo "$v_extracted $b"
    done | LC_ALL=C sort -V | awk '{print $2}' ### Sort by version column, print bundle column
}
...
### Generate YAML Contents (Prioritizes Default Channel, then Cascades)
set_contents() {
    ...
    for op in "${lc_ops[@]}"; do
        ...
        mapfile -t spec_versions < <(printf '%s\n' "${parts[@]:1}" | sort -Vr)
        ...
        ### 4. Start Writing YAML Block for Operator
        cat <<EOF >> "$lc_yaml"
    - name: $op_name
      channels:
EOF

        ### 5. Strategy: Default First, Then Others
        local -a remaining_specs=("${spec_versions[@]}")
        local channel_matches_found="false"

        ### Case A: No specific versions requested (Full Default Channel)
        if [[ ${#spec_versions[@]} -eq 0 || ( ${#spec_versions[@]} -eq 1 && -z "${spec_versions[0]}" ) ]]; then
            printf "%-8s%-80s\n" "[INFO]" "       Adding Default Channel (Full): $def_chan"
            cat <<EOF >> "$lc_yaml"
...
EOF
            ...
            if [[ ${#found_in_def[@]} -gt 0 ]]; then
                ...
                # printf "%-8s%-80s\n" "[DEBUG]" "       Matches: ${found_in_def[*]}"

                cat <<EOF >> "$lc_yaml"
      - name: $(get_string "$def_chan")
EOF
                ### Sort using Version Sort (sort -V) logic via helper
                ...
                ### Default Channel Logic: Only write minVersion
                [[ -n "$min_ver_str" ]] && echo "        minVersion: $(get_string "$min_ver_str")" >> "$lc_yaml"

                channel_matches_found="true"

                ### Remove found versions from remaining_specs
            ...
            ### Step 5-2: Check Other Channels (if specs remain)
            if [[ ${#remaining_specs[@]} -gt 0 ]]; then
                for channel in "${all_channels[@]}"; do
                    ### Skip default channel (already processed) and candidate
...
EOF
                        ...
                        ### Other Channels Logic: minVersion + maxVersion
                        [[ -n "$min_ver_str" ]] && echo "        minVersion: $(get_string "$min_ver_str")" >> "$lc_yaml"
