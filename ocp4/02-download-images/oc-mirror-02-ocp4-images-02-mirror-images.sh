...
### ---------------------------------------------------------------------------------
### This script mirrors OpenShift Container Platform (OCP) release images using the 'oc-mirror' tool and previously generated ImageSet configurations.
...
### Main Execution
### ---------------------------------------------------------------------------------
### Parse OCP versions from the configuration to iterate through them.
extract_ocp_versions

printf "%-8s%-80s\n" "[INFO]" "=== Starting Mirroring for OpenShift Release Images ==="
...
for major_minor_patch in "${OCP_VERSION_ARRAY[@]}"; do
    ...
    if [[ -d "$oc_mirror_work_dir/working-dir" ]]; then
        ### Clean up the temporary working directory from any previous runs.
    ...
    if [[ -f "$imageset_config_file" ]]; then
        printf "%-8s%-80s\n" "[INFO]" "--- Mirroring OpenShift Release Images for OCP Version: $major_minor_patch ..."
...
### Provide a final summary of the completed mirroring tasks.
echo ""
printf "%-8s%-80s\n" "[INFO]" "=== OpenShift Release Image Mirroring Process Completed ==="
...
for export_dir in "${mirror_export_dirs[@]}"; do
    ...
    ls -lh "$export_dir" | grep -v "^total" || {
        printf "%-8s%-80s\n" "[ERROR]" "Failed to list files in '$export_dir'. Exiting..."
        exit 1
    }
done
echo ""