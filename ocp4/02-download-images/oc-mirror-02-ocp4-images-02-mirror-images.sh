...
if [[ ! -f "./oc-mirror" ]]; then
    printf "%-8s%-80s\n" "[ERROR]" "    The 'oc-mirror' binary was not found at './oc-mirror'." >&2
    exit 1
fi
...
mirror_images() {
    ...
    ### Define the command and arguments as an array (Safe for spaces)
    local CMD_ARGS=(
        "./oc-mirror"
        "--v2"
    ...
    ### Construct a string for logging purposes
    local CMD_STRING="${CMD_ARGS[*]}"

    ### Log the parameters and the full command
    printf "%-8s%-80s\n" "[INFO]" "    --------------------------------------------------"
    printf "%-8s%-80s\n" "[INFO]" "    Parameters:"
