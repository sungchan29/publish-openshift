...
validate_non_empty "OPENSHIFT_CLIENT_DOWNLOAD_URL" "$OPENSHIFT_CLIENT_DOWNLOAD_URL"
validate_non_empty "OC_MIRROR_DOWNLOAD_URL" "$OC_MIRROR_DOWNLOAD_URL"
validate_non_empty "BUTANE_DOWNLOAD_URL" "$BUTANE_DOWNLOAD_URL"
...
printf "%-8s%-80s\n" "[INFO]"     "    Butane Tool       : $BUTANE_DOWNLOAD_URL"
if [[ -n $OPENSHIFT_CLIENT_RHEL8_TAR ]]; then
    printf "%-8s%-80s\n" "[INFO]" "    OCP Client (RHEL8): $OPENSHIFT_CLIENT_DOWNLOAD_URL/$latest_version/$OPENSHIFT_CLIENT_RHEL8_TAR"
fi
if [[ -n $OPENSHIFT_CLIENT_RHEL9_TAR ]]; then
    printf "%-8s%-80s\n" "[INFO]" "    OCP Client (RHEL9): $OPENSHIFT_CLIENT_DOWNLOAD_URL/$latest_version/$OPENSHIFT_CLIENT_RHEL9_TAR"
fi
...
if [[ -n $PIPELINES_CLI_DOWNLOAD_URL ]]; then
    printf "%-8s%-80s\n" "[INFO]" "    Pipelines CLI Tool: $PIPELINES_CLI_DOWNLOAD_URL"
fi
...
if [[ -n $OPENSHIFT_CLIENT_RHEL8_TAR ]]; then
    download_tool "$OPENSHIFT_CLIENT_DOWNLOAD_URL/$latest_version/$OPENSHIFT_CLIENT_RHEL8_TAR"
fi
if [[ -n $OPENSHIFT_CLIENT_RHEL9_TAR ]]; then
    download_tool "$OPENSHIFT_CLIENT_DOWNLOAD_URL/$latest_version/$OPENSHIFT_CLIENT_RHEL9_TAR"
fi
if [[ -n $OC_MIRROR_RHEL8_TAR ]]; then
...
if [[ -n $PIPELINES_CLI_DOWNLOAD_URL ]]; then
    download_tool "$PIPELINES_CLI_DOWNLOAD_URL"
fi
