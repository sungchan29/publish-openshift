        ...
        if [[ -n "$NODE_ROLE_SELECTORS" && "$all_labels_applied" = "false" ]]; then
            ...
            if [[ -n "$node_label_trigger_search_result" ]]; then
                ...
                for node_role_selector in $NODE_ROLE_SELECTORS; do
                    ...
                    # Disable set -e for oc commands to prevent script exit
                    set +e
                    nodes=$(timeout 5s ./oc get nodes --no-headers -o custom-columns=":metadata.name" 2>/dev/null | egrep -E "${node_name}|${node_name}\.${CLUSTER_NAME}\.${BASE_DOMAIN}" || true)
                    exit_code=$?
                    set -e
