...
if [[ "$MIRROR_REGISTRY_USERNAME" != "$(podman login --get-login "$MIRROR_REGISTRY" 2>/dev/null)" ]]; then
    ...
    if [[ -t 0 ]]; then
        ...
        podman login -u "$MIRROR_REGISTRY_USERNAME" "$MIRROR_REGISTRY"
    else
...
### MIRROR_STRATEGY defaults to 'incremental' if empty
