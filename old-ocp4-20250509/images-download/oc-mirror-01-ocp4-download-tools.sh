...
if [[ ${#OCP_VERSION_ARRAY[@]} -eq 0 ]]; then
    log "ERROR" "OCP_VERSION_ARRAY is empty. Run extract_ocp_versions first."
    exit 1
fi
latest_version="${OCP_VERSION_ARRAY[-1]}"
if [[ -z "$latest_version" ]]; then
    log "ERROR" "Failed to determine latest OCP version."
    exit 1
fi
...
echo "OPENSHIFT_CLIENT_RHEL8_FILE : $OPENSHIFT_CLIENT_DOWNLOAD_URL/$latest_version/$OPENSHIFT_CLIENT_RHEL8_FILE"
echo "OPENSHIFT_CLIENT_RHEL9_FILE : $OPENSHIFT_CLIENT_DOWNLOAD_URL/$latest_version/$OPENSHIFT_CLIENT_RHEL9_FILE"
